...
define(
	...
	function(
		 ...
		 * @constructor
		 */
		var Map = function(server, subdomains, canvas, zoom, coordinates, settings){
			...
			this.fully_rendered_callback = null;
			...
			if(settings){
				if(settings.rendering_enabled)
					this.rendering_enabled = settings.rendering_enabled;
			}
		...
		Map.prototype._update_fully_rendered = function(fully_rendered){

			if(fully_rendered && this.fully_rendered_callback !== null)
				this.fully_rendered_callback();
		...
		Map.prototype.fit_objects = function(){

			var i, bounds, tileWidth, tileHeight, center, centerXY, centerLL;
			...
			for(i = 0; i < this.objects.length; i++){
				if(this.objects[i].is_visible()){
					...
					if(bounds === null)
						bounds = this.objects[i].bounds();
					else
						bounds = bounds.combine(this.objects[i].bounds());
				}
			}

			tileWidth = this.canvas.width / Map.TILE_SIDE_LENGTH / 2;
			tileHeight = this.canvas.height / Map.TILE_SIDE_LENGTH / 2;

			center = new LatLong(
				bounds.position.y + (bounds.dimension.h / 2),
				bounds.position.x + (bounds.dimension.w / 2)
			);

			for(i = 18; i > 0; i--){

				centerXY = Location_Conversions.latlong_to_tilexy(center, i);
				centerXY.x -= tileWidth;
				centerXY.y += tileHeight;
				centerLL = Location_Conversions.tilexy_to_latlong(centerXY, i);

				if(centerLL.latitude < bounds.position.y && centerLL.longitude < bounds.position.x)
					break;

			}

			this.zoom = i;
			this.center_at(center);
		...
		Map.prototype.load_all_tiles = function(tileFinished){
		...
		Map.prototype._do_render = function(){
			...
			this._update_fully_rendered(fully_rendered);
